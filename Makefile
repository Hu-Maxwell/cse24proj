...
LDFLAGS = -lfltk_images -lpng -lz -lfltk_gl -lGLU -lGL -lfltk -lXrender \
          -lXext -lXft -lfontconfig -lpthread -ldl -lm -lX11 -lglut
...
SRC = $(wildcard $(SRC_DIR)/*.cpp) $(wildcard $(SRC_DIR)/core/*.cpp) $(wildcard $(SRC_DIR)/tools/creators/*.cpp) $(wildcard $(SRC_DIR)/tools/modifiers/*.cpp $(wildcard $(SRC_DIR)/tools/creators/shapes/*.cpp))  $(wildcard $(SRC_DIR)/utils/*.cpp) $(wildcard $(SRC_DIR)/tools/helpers/*.cpp)
